	...
	function insertData($conn, $contacts, $filename) {
		foreach ( $contacts as $contact ) {
			$insertSql = "INSERT INTO contacts_dump(`src_id`, `src_con_id`, `con_first_name`, 
						...
						VALUES ('" . $_SESSION['username'] . "','" . $contact->src_con_id . "','" . $contact->con_first_name . "','" . $contact->con_middle_name . "','" . $contact->con_last_name . "','" . $contact->con_salutation . "','" . $contact->con_phone_no . "','" . $contact->con_fax_no . "','" . $contact->con_country . "','" . $contact->con_zipcode . "','" . $contact->con_email . "',
								STR_TO_DATE('$contact->con_created_date', '%c/%d/%y'),'".$_SESSION['username']."');";
